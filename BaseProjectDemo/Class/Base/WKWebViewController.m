...
-(void)enterActivity{
    //充值状态提示框
   // [[AHPurchaseGoodsTool share]showTopUpStatusHUB];
}
...
- (void)webView:(WKWebView *)webView decidePolicyForNavigationAction:(WKNavigationAction *)navigationAction decisionHandler:(void (^)(WKNavigationActionPolicy))decisionHandler {
    ...
    if ([url containsString:@"alipay://"]) {
        url = [url stringByReplacingOccurrencesOfString:@"alipays" withString:@"NiuNiuLivet"];
    ...
    if([url containsString:@"weixin://"]){
        url = [url stringByAppendingString:@"&appid=wx66e221daac64db0a"];
...
- (WKWebView *)wkWebView{
    if (!_wkWebView) {
        ...
        _wkWebView = [[WKWebView alloc] initWithFrame:CGRectMake(0, 0, DEVICE_SCREEN_WIDTH, DEVICE_SCREEN_HEIGHT-74) configuration:Configuration];
...
-(UIBarButtonItem*)customBackBarItem{
    if (!_customBackBarItem) {
        UIImage* backItemImage = [[UIImage imageNamed:@"btn_home_arrow"] imageWithRenderingMode:UIImageRenderingModeAutomatic];
        UIImage* backItemHlImage = [UIImage imageNamed:@"btn_home_arrow"];
        UIButton* backButton = [[UIButton alloc] init];
        backButton.frame = (CGRect ){.size= {backItemImage.size.width,backItemImage.size.height}};
