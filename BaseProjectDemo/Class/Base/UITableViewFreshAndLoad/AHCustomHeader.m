...
/**
 使用gif图片加载
 */
-(void)UseGifImageLoadingWihtName:(NSString*)imageNameStr{
    _isGifAnimation = YES;
    self.mj_h = 100;
    NSString  *filePath = [[NSBundle bundleWithPath:[[NSBundle mainBundle] bundlePath]] pathForResource:imageNameStr ofType:nil];
...
- (void)placeSubviews
{
    ...
    if (_isGifAnimation) {
        ...
        self.logo.center = CGPointMake(DEVICE_SCREEN_WIDTH/2, self.mj_h * 0.5 - 5);
       self.label.center = CGPointMake(DEVICE_SCREEN_WIDTH/2, self.mj_h * 0.5+30);
    }else{
        self.label.frame = self.bounds;
        self.label.center = CGPointMake(DEVICE_SCREEN_WIDTH/2, self.mj_h * 0.5+20);
        self.logo.center = CGPointMake(DEVICE_SCREEN_WIDTH/2, self.mj_h * 0.5 - 5);
    }
}
